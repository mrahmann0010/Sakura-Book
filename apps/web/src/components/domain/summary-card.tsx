...
export function SummaryCard({
  ...
  return (
    <Card variant="tint" padding="roomy" as="section" className={className}>
      ...
      <div className="mt-5.5 flex flex-col gap-3.5 text-13.5">{children}</div>
      {footer}
    </Card>
  );
}
...
export function SummaryRow({
  ...
  return (
    <div className={cn(summaryRow({ tone }), className)}>
      <span className={tone === "default" ? "text-secondary" : undefined}>
        {label}
      </span>
  ...
  return (
    ...
    >
      ...
      <p
        className={cn(
          "min-w-0 truncate",
          isSmall ? "text-13 text-ink" : "font-serif text-17 text-ink",
        )}
      >
        {book.title}
