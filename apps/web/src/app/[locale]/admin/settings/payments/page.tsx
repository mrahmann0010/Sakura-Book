...
export default function AdminPaymentSettingsPage() {
  ...
  const [meta, setMeta] = useState<Pick<AdminPaymentNumbers, "source" | "updatedAt" | "updatedByEmail"> | null>(
    null,
  );
  const [form, setForm] = useState<FormState>({ bkashNumber: "", rocketNumber: "", nagadNumber: "" });
  ...
  useEffect(() => {
    ...
    getAdminPaymentNumbers()
      ...
      .catch((cause: unknown) => {
        if (cancelled) return;
        setError(cause instanceof AdminApiError ? cause.message : "Could not load payment settings.");
      })
      .finally(() => {
  ...
  return (
    <AdminShell checking={checking}>
      <div className="flex max-w-lg flex-col gap-6">
        <div>
          <h1 className="text-h2 text-ink font-serif">Payment Settings</h1>
          <p className="text-13.5 text-secondary mt-1">
            The bKash, Rocket, and Nagad numbers customers see at checkout. Changing them here is
            the only way to change them — checkout only displays and copies these, it cannot edit
            them.
          </p>
        </div>
